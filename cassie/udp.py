...
class CassieUDPController:
  def __init__(self, env):
    ...
    self.clock_based = env.clock_based
    self.no_delta = env.no_delta
    self.env = env

    u = pd_in_t()
    for i in range(5):
        u.leftLeg.motorPd.pGain[i] = self.env.P[i]
        u.leftLeg.motorPd.dGain[i] = self.env.D[i]
        u.rightLeg.motorPd.pGain[i] = self.env.P[i]
        u.rightLeg.motorPd.dGain[i] = self.env.D[i]
    ...
    self.standing_height = 0.7
    self.MAX_HEIGHT = 0.8
    self.MIN_HEIGHT = 0.4
    self.D_mult = 1  # Reaaaaaally bad stability problems if this is pushed higher as a multiplier
                     # Might be worth tuning by joint but something else if probably needed

  def control_loop(self, policy):
    while True:
      ...
      #print("got an action: {}".format(action))
      #input()
...
def run_udp(args):
  ...
  udp = CassieUDPController(env)

  action = 0
  loop = udp.control_loop(policy)
